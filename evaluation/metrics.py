...
def sample(batch_size:int, maskgit, device, n_samples: int, static_condition):

    # sampling
    x_new_l, x_new_h, x_new = static_condition_sample(maskgit, n_samples, device, static_condition, batch_size)  # (b c l); b=n_samples, c=1 (univariate)
...
class Metrics(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, maskgit, device, n_samples: int, static_condition):
        return sample(self.batch_size, maskgit, device, n_samples, static_condition)
