...
class MaskGIT(nn.Module):
    ...
    def masked_prediction(self, transformer, static_condition, *s_in):
        """
        masked prediction with static conditions
        """
        logits = transformer(*s_in, static_condition=static_condition)  # (b n k)
        return logits
    ...
    def first_pass(self,
        ...
        for t in range(self.T['lf']):
            logits_l = self.masked_prediction(self.transformer_l, static_condition, s_l)  # (b n k)
    ...
    def second_pass(self,
        ...
        for t in range(self.T['hf']):
            logits_h = self.masked_prediction(self.transformer_h, static_condition, s_l, s_h)  # (b m k)
    ...
    @torch.no_grad()
    def iterative_decoding(self, num=1, mode="cosine", static_condition=None, device='cpu'):
        """
        ...
        # static_condition (b, num_static_condition, 1)
        s_l = self.first_pass(s_l, unknown_number_in_the_beginning_l, static_condition, gamma, device)
        s_h = self.second_pass(s_l, s_h, unknown_number_in_the_beginning_h, static_condition, gamma, device)
        return s_l, s_h
